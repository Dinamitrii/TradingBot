...
def determine_buy_event(symbol, timeframe, percentage_rise):
    ...
    if candlestick_data.loc[0, 'RedOrGreen'] == "Green" and candlestick_data.loc[1, 'RedOrGreen'] == "Green" and \
            candlestick_data.loc[2, 'RedOrGreen'] == "Green":
...
def strategy_one(timeframe, percentage_rise, quote_asset, project_settings):
    ...
    while 1:
        ...
        if current_time != previous_time:
            ...
            new_trading_symbols = analyze_symbols(symbol_dataframe=asset_list, timeframe=timeframe,
                                                  percentage_rise=percentage_rise)
            ...
            for symbol in new_trading_symbols:
                if symbol not in trading_symbols:
                    ...
                    trade_parameters = calculate_trade_parameters(symbol=symbol, timeframe=timeframe,
                                                                  asset_list=asset_list)
                    # Make a trade
                    try:
                        trade_outcome = binance_interaction.make_trade_with_params(params=trade_parameters,
